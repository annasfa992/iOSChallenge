...
extension LogManager {
    ...
    class func res<T: Codable>(_ response: ResponseModel<T>) {
        ...
        let _: String = response.copyright
        let _: Int? = response.numResults
        ...
        if let path = path {
            log = log + "[\(ServiceManager.shared.baseURL)\(path)]\n"
        }
        
        log = log + "[status: \(status)]\n[msgWithLanguage: \(status ?? "")]"
    ...
    class func err(_ error: ErrorModel) {
        ...
        let errorKey: Int? = error.errorKey
